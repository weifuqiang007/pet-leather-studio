...
### R3 复验完成（GLM，2026-09-22）

用户裁定 R2 工程诊断通过但视觉验收仍不通过（验收报告 c5443d5 提出 R3 四点），已全部实现：① GUI 过渡带措辞改"理想裙边建议 ≥ X mm（不含输入深度断层）"，`domain/photo_relief.py` 新增 `slope_exceedances`（全域/边界/域内实测 >45° 逐项列出），母版 warnings 与详情 ⚠ 行明示"加宽带宽无效，建议平滑/降起伏"；② smoothing report 关闭时也落盘 `{"enabled": false}`（可审计），开启时新增细节保留率代理 detail_retention；③ 新增 `RECOMMENDED_MAX_RELIEF_MM=4.0`（皮雕挂件工程启发值，超限警告不拒绝，指向显式降起伏而非加宽裙边）；④ 新实验 `run_p2_r3_smoothing_matrix.py`（显式 2.0 / 比例 8.352 × 平滑 0/1.0/1.5/2.0 共 8 臂）与 `run_p2_r3_three_sample.py`（短毛犬/猫/长毛犬同条件：显式 2.0 + 带宽 3.0 + 平滑 1.5，各正/侧/斜三视图）。**诚实结论：比例 8.35 mm 即使平滑 2.0 全域仍 73°（裙带内 EDT 最近邻切换的单格悬崖，平滑/加带都不消），不建议进模具；显式 2.0 + 3.0 + 1.5 三样本全达标零警告**（全域 ≤44.1°、细节保留 0.60–0.81）。测试计数已按提交说明（R1 161 / R2 168 / R3 **173 passed**，+5；旧测试零删除），ruff/mypy 通过，GUI smoke 复跑 ok。新标签 `photo-relief-p2-r3`（旧标签未动）；视觉验收（三样本逐一无贴图确认）归用户，通过后方可进入阴阳模与皮革试压，`visual_review` 维持 pending。详细数据见交付报告"复验记录（R3）"节。
