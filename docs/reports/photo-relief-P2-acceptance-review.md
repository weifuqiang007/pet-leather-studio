...
## R3 复验（757605d，2026-09-22）

### 结论

R3 完成了上一轮要求的工程修正：测试计数已澄清，真实坡度/起伏警告、可审计平滑、比例高度平滑矩阵和三样本同条件渲染均已落地。**工程验收通过；产品视觉验收仍不通过。** 不应把“explicit 2.0 mm + 带宽 3.0 mm + 平滑 1.5 mm”设为工程默认，也不应进入阴阳模与试压。

### 可复算结果

在 `757605d` 的工作树以统一命令 `scripts/dev.sh run --frozen python -m pytest tests -m 'not real_model'` 实跑为 **173 passed, 2 deselected**；Ruff、格式检查和 mypy 均通过。此前 R2 的 161/168 争议已得到合理解释：当前提交包含 R2 的 7 项和 R3 的 5 项新增测试。

本机重读 R3 平滑矩阵和三样本总账，记录与交付报告一致：

1. 比例高度 8.352 mm 的四个平滑档均保留坡度/起伏超限警告；即使 2.0 mm 平滑，全域最大坡度仍为 73.0°。该路径不适合进入模具。
2. 三样本候选组合的数值门均通过：短毛犬/猫/长毛犬的全域最大坡度分别为 44.1°、39.2°、43.8°，warnings 均为空。`smoothing` 和细节保留率也已写入各自 manifest。
3. GUI 对“理想裙边建议”与实测超限的措辞已修正；这避免了把解析裙边公式误说成成品真实坡度。

### 未通过的视觉复核

实际检查 R3 三样本的无贴图 ISO、正面和侧面渲染后，数值合格不等于产品合格：

- 短毛犬、猫和长毛犬都只留下了很浅、模糊的大轮廓；眼、鼻、耳、毛流等应成为皮雕识别点的几何细节没有形成清晰的层级。
- 三个 ISO 图仍沿主体边界可见锯齿状/条状沟槽。它们虽不再是高而直的墙，却也不是可直接用于皮革压制的自然轮廓。
- 猫样本的细节保留率为 0.600，渲染中已经明显变柔；这与报告的“平滑细节代价”一致。用单一 1.5 mm 高斯平滑作为所有题材的默认值，会进一步牺牲毛发、五官、花瓣脉络等目标细节。

因此，R3 证明了“低起伏 + 平滑”能满足当前坡度阈值，却没有实现项目所需的“照片输入后生成具有真实关键结构和纹理细节的浮雕母版”。视觉门应继续为 pending，不能由用户仅在 GUI 勾选而绕过该缺陷。

### 后续实现方向

下一阶段应把“控制坡度”和“表达结构”分离：先从图像/深度生成可编辑的结构层（主体体块、眼鼻口/耳朵、毛流或花瓣脉络），分别分配高度、边缘宽度和局部平滑；再将细节层以受坡度上限约束的方式叠加到平滑的体块层。验收样本必须换为足够分辨率的真实照片，并与原图并排检查关键部位是否保留，而不只检查 heightfield 是否连续。

R3 标签 `photo-relief-p2-r3` 指向 `757605d`；本节对应的本机渲染仍为 gitignored 证据，不能由 Git 单独复核视觉结论。
