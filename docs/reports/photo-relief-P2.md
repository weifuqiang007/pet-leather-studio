...
## 复验记录（R3：比例高度平滑对照、真实坡度警告、起伏建议上限与三样本复验，2026-09-22）

R2 复验后用户裁定：**工程诊断能力通过，但视觉验收仍不通过**，不能进入阴阳模和皮革试压（验收结论与 R3 必修四点见 [photo-relief-P2-acceptance-review.md](photo-relief-P2-acceptance-review.md) 提交 c5443d5），另要求更正/说明交付报告所用测试集合。R3 已全部实现并重跑真机实验。

### 测试计数更正与说明（验收前置要求）

- 统一命令：`scripts/dev.sh run --frozen python -m pytest tests -m 'not real_model'`。
- 按提交实测：R1（8b9a6ad，tag `photo-relief-p2-r1`）**161 passed, 2 deselected**；R2（83ae10c，tag `photo-relief-p2-r2`）与用户复验提交 c5443d5 上均为 **168 passed, 2 deselected**（R2 在 R1 基础上 +7：unit 4 + integration 1 + GUI 2，旧测试零删除）。R2 报告写 168 与代码一致，非虚报。
- 复验得 161 的解释：161 恰为 R1 时代测试集计数（缺 R2 的 +7），推测复验运行发生在尚未包含 R2 测试的工作树/环境上；同一命令在 83ae10c 与 c5443d5 复跑两遍均得 168。
- 本轮（R3）：**173 passed, 2 deselected**（新增 +5：unit 2 + integration 1 + GUI 2；构成 unit 94 + integration 56 + GUI 19 + 架构等 4）。

### 实现内容（对应验收四点）

1. **措辞与真实坡度警告**（验收第 1 点）：GUI 过渡带提示改为"理想裙边建议 ≥ X mm（裙边公式最陡 ≤45°，**不含输入深度断层**；实测以母版详情坡度行为准）"；domain 新增 `slope_exceedances(slope)`（全域/边界过渡/域内任一实测 >45° 即逐项列出，+1e-9 容差）；母版 metadata warnings 与 GUI 母版详情在超限时给出 ⚠ 行，明示"由输入深度固有断层主导；建议加平滑半径或降低起伏，**加宽带宽无效**"。
2. **平滑可审计 + 比例高度对照**（验收第 2 点）：`controlled_heights_mm` 的 smoothing report **关闭时也落盘 `{"enabled": false}`**（不能缺省成"未记录"）；开启时新增细节保留率代理 `detail_retention = 1 − 位移RMS/起伏RMS`（有效域内单位高度口径；数值代理，不替代视觉评审）。新实验①对短毛犬同 depth 修订跑 2 高度模式（显式 2.0 / 参考比例 8.352）× 4 平滑档（0/1.0/1.5/2.0）= 8 臂——R2 只验证过显式 + 1.5 一臂，本轮补齐比例模式全档。
3. **合理起伏上限**（验收第 3 点）：domain 新增 `RECOMMENDED_MAX_RELIEF_MM = 4.0`（60 mm 级皮雕挂件工程启发值，非物理推导）：解析起伏超过即在 GUI 表单与 manifest warnings 提示"建议改显式深度降低起伏或收窄宽度，而不是加宽裙边"；**只警告不拒绝**（硬范围仍为 DEPTH_MM_RANGE 0.05–20 mm）。
4. **三样本同条件对照**（验收第 4 点）：新实验②以短毛犬/猫/长毛犬三张真实样本、同参数（显式 2.0 mm + 建议带宽 3.0 mm + 平滑 1.5 mm）各建母版，渲染**正面/侧面/斜视**无贴图三视图供用户逐一确认（通过条件：无硬墙/条状断层、关键轮廓清晰，且用户在 GUI 亲自确认后方可进入阴阳模与试压）。

### 实验①数据：平滑矩阵（短毛犬 depth 8a18d7ed…；带宽=各自模式建议值，显式 3.0 / 比例 12.6）

| 臂 | 深度 mm | 平滑 mm | 全域° | 边界° | 域内° | 细节保留 | 版边抬升 mm | 警告 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| explicit-s0.0 | 2.0 | – | 71.4 | 70.4 | 71.4 | – | 2.00 | 坡度超限（三项） |
| explicit-s1.0 | 2.0 | 1.0 | 52.0 | 19.0 | 16.0 | 0.693 | 1.98 | 坡度超限（全域） |
| **explicit-s1.5** | 2.0 | 1.5 | **44.1** | **15.2** | **10.5** | 0.632 | 1.95 | **无** |
| explicit-s2.0 | 2.0 | 2.0 | 35.2 | 13.5 | 7.8 | 0.576 | 1.90 | 无 |
| ratio-s0.0 | 8.352 | – | 85.5 | 85.4 | 85.4 | – | 8.35 | 坡度 + 超上限 |
| ratio-s1.0 | 8.352 | 1.0 | 81.4 | 57.3 | 50.1 | 0.693 | 8.26 | 坡度 + 超上限 |
| ratio-s1.5 | 8.352 | 1.5 | 77.5 | 39.3 | 37.9 | 0.632 | 8.13 | 坡度 + 超上限 |
| ratio-s2.0 | 8.352 | 2.0 | 73.0 | 26.3 | 29.7 | 0.576 | 7.93 | 坡度 + 超上限 |

（母版修订 id 依次 `762b0716/17debc89/db4d69f8/65ca8612`、`0e0f6d80/6e0e0679/54fa1a6b/7e28a193`…，修订史 append-only。）

### 实验②数据：三样本同条件（显式 2.0 mm × 带宽 3.0 mm × 平滑 1.5 mm）

| 样本 | 全域° | 边界° | 域内° | 细节保留 | 警告 | 母版修订 |
| --- | --- | --- | --- | --- | --- | --- |
| 短毛犬 | 44.1 | 15.2 | 10.5 | 0.632 | 无 | `9c37b28a`… |
| 猫 | 39.2 | 25.5 | 9.2 | 0.600 | 无 | `bef3f6cc`… |
| 长毛犬 | 43.8 | 16.3 | 8.6 | 0.809 | 无 | `bdb68708`… |

### 诚实结论

1. **比例高度模式不建议进入模具**：8.35 mm 起伏即使平滑 2.0 mm，全域实测仍 73.0°（>45° 目标，全部比例臂都带坡度警告）。残余陡点在裙边背景带内——相邻裙边单元的"最近有效高度"不同源（EDT 最近邻 Voronoi 切换），平滑与加宽带宽都不能消除该类单格悬崖。且 8.35 mm 同时触发建议上限警告。**压制级母版建议显式深度 ≤4.0 mm**。
2. **显式 2.0 + 建议带宽 3.0 + 平滑 1.5 达标且零警告**：全域 44.1° 即裙边公式界（1.5×2/3 → 45° 以内），三样本同条件全部满足（全域 ≤44.1°、边界 ≤25.5°、域内 ≤10.5°）。
3. **平滑的细节代价随半径单调上升**（保留率 0.693 → 0.632 → 0.576）；猫样本 1.5 档渲染目视已偏"柔"（保留率 0.60，细节被抹平的权衡可见）。档位取舍属审美裁决，归用户。
4. **数值门与视觉门分开陈述**：ratio-s2.0 侧视渲染目检轮廓平缓落地、无竖直墙——与全域 73° 并不矛盾（残余陡点为局部单格对、幅度小，最大坡度指标对其敏感；视觉不显眼不代表数值达标）。通过标准仍以数值 + 用户逐样本 GUI 目验双门为准。

### 验证

- 测试 +5（unit 2：`slope_exceedances` 只标超限项/平滑报告可审计且保留率随半径单调；integration 1：悬崖场母版警告文案 + smoothing 落盘；GUI 2：表单超上限 ⚠ 与版边余量收窄、母版详情 ⚠ 坡度行）→ **173 passed, 2 deselected**；ruff check / format / mypy（domain+application）通过；真实模型 2 项未重跑（R3 不触及推理内核）。
- GUI 真机 smoke 复跑 ok（`runtime/gui_p2_smoke.py`）：过渡带提示含"理想裙边建议 ≥ 12.6 mm … 不含输入深度断层"与"⚠ 起伏 8.35 mm 超过建议上限 4.0 mm"；证据截图已更新至 `runtime/evidence/photo-p2/`。
- 渲染目检：ratio-s2.0 与猫样本侧视图（远程视觉模型复核）确认落地平缓、无竖直断层/条状沟槽；猫样本确认顶面细节已柔化（与保留率 0.60 一致）。

### 版本与剩余裁决

- R3 提交见 `git log` 本条（feat(p2-r3)）；新标签 **`photo-relief-p2-r3`**（只新增；`photo-relief-p2-r2` 及更早标签未动）。
- **视觉验收归用户**（验收第 4 点）：请在 GUI 逐一确认三样本（渲染在 `experiments/photo_relief/out/p2-r3-three-sample/<key>/`，各 正面/侧面/斜视 三视图；矩阵对照在 `out/p2-r3-smoothing-matrix/`，16 张侧视/斜视）；确认通过后即可把显式 2.0 + 带宽 3.0 + 平滑 1.5 设为工程默认进入模具链。`visual_review` 维持 **pending**（按用户指示本轮不代勾）。
- 数据与渲染路径：`experiments/photo_relief/out/p2-r3-smoothing-matrix/report_data.json`、`out/p2-r3-three-sample/report_data.json` 及各 png（gitignored，本机留存）。
