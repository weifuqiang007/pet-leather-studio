...
> 注：本轮蒙版为 M0.5 局部标注（评审 R3 指出覆盖不全），已被下方"PH09 复跑"取代，后者又被第三、四轮取代；本轮数据与证据原样保留。**当前阅读入口：第四轮（`20260921-110433`）。**
...
- 证据：`runtime/evidence/photo-p1/ph09-r3-20260921-100609-<key>-{mask-overlay,iso}.png`（已被 r4 证据取代并移除，out/ 机器数据保留）。

## 第三轮复验（2026-09-21，回应复验报告 4ed7c05）

背景：第三方第三轮验收（`docs/reports/photo-relief-P1-reacceptance-4ed7c05.md`）结论：F1/F4 与 F2 主路径可关闭，另提出 A/B/C。逐项处置：

| 项 | 评审发现 | 处置 | 验证 |
| --- | --- | --- | --- |
| A [P2] 损坏 selected.json 被当"没有选择" | JSON 解析失败/缺 revision/空值返回 None，locate 静默回退最新 | `_selected` 区分文件不存在（正常回退）与存在但损坏：无法解析、非对象、缺 `revision`、空值/类型错误均抛 `ResourceMissingError`，错误信息附修复指引 | `test_registry_corrupted_selected_errors_not_fallback`（4 参数化：损坏 JSON/缺 revision/空 revision/非对象） |
| B [P2] 叠加图未保留主体处原图 RGB | `paste` 用 255 蒙版整片替换像素，输出纯红 RGB + alpha=80，主体内部是"红色×查看器底色" | 改 `alpha_composite`：红色 α=80/255 真正混入原图 RGB，边界黄线，成品存 RGB（无 alpha） | `test_overlay_blends_red_into_original_rgb`（像素级断言 ≈0.69×原图+0.31×红、非恒定纯红、蒙版外原图、边界黄线、输出 RGB） |
| C [P2] IoU 自洽≠主体正确；三处可疑 | 短毛犬"颈胸缺口"、猫右侧矩形突出、长毛犬腹部挖空 | 逐处回原图判定（见下）；mask_method 改 `threshold_assisted` 并在 notes 记录判据参数与人工修正 | 色度/阈值像素证据 + 修正后叠加图视觉复核 |

C 项三处的判定证据与处置：

- **猫右下矩形（x≈128-147, y≈114-138）= 误纳，剔除**。像素证据：该区中性灰 [161,158,157]（R≈G≈B）而猫毛暖调 [157,134,126]（R−B=31）；为 gettyimages 白字水印叠灰影，非身体。第四轮改用色度判据（R−B>12 且 dist>18）重测轮廓，右缘沿暖色毛 (127,89)→(125,110)→(138,144) 直落底部毛条带右端，灰影自然消失。
- **长毛犬腹/腿间 = 遗漏，补回**。争议三区（x118-141 y106-135 等）实测中位 dist 35-39、**0% 纯白**（对照纯背景 100% 纯白）——是浅色毛渐变而非透光空隙；第三轮 thr40 判据过深挖出锯齿凹口。第四轮改 thr35 主域∪thr40 主域+补洞，腹线连续，后腿下缘 (159,134)→(150,135)→(144,133) 一并保留（简化容差收到 eps=1.0、IoU 0.991，避免 DP 抹掉后腿下缘 27px 毛）。
- **短毛犬"颈胸缺口" = 误读，不改**。增强图+色度右沿（y80-147 稳定 x≈111-119）与当前多边形（x≈114-120）±3px 一致；缺口实为 B 缺陷（纯红叠加图不可核对）造成的视觉误判。三重证据（色度/thr35/thr50）一致。

第四轮 PH09（`workspace/photo-relief-p1/20260921-110433/`，MPS 真实推理）：

| 样本（蒙版口径） | 蒙版覆盖（三轮→四轮） | 深度有效 | 推理 | 全程 |
| --- | --- | --- | --- | --- |
| 短毛犬（头 + 可见上身） | 37.4% → 37.4%（未改） | 37.4% | 1.20 s | 6.0 s |
| 猫（剔除右下灰影水印） | 63.0% → 61.4% | 61.4% | 0.99 s | 6.1 s |
| 长毛犬（补回腹/腿间浅毛） | 48.9% → 51.3% | 51.3% | 0.85 s | 4.5 s |

- 逐行/逐列外沿核对：短毛犬残差同第三轮（≤6px 不连通绒毛簇）；猫的"带外偏差"全部位于被有意剔除的灰影矩形与左下 credit 文字（亮度判据与色度判据的预期差异）；长毛犬残差为顶点扫描退化与 ≤5px 绒毛渐变。
- 叠加图（B 修复后）经视觉模型逐面板复核：红色区内部五官/毛流可辨；猫灰影排除、胸口保留；长毛犬腹毛纳入、无挖空。`visual_review` 仍 pending 待用户勾选。
- 证据：`runtime/evidence/photo-p1/ph09-r4-20260921-110433-<key>-{mask-overlay,iso}.png`。

第三轮复验测试计数（本机 macOS arm64）：ruff check / format（68 文件）/ mypy 通过；`pytest tests -m 'not real_model'` **112 项通过**（较上轮 +5：A×4、B×1）；真实模型 2 项上轮通过、本轮未重跑（A/B/C 改动不触及推理内核）。
