...
**状态**：M1 软件几何验收已通过（2026-09-23，标签 `leather-mold-m1-r2`）。R2 使用“点到三角面距离减采样界”的 `conservative_min_mm` 作为唯一放行门，三份真实候选均过门；详见 `docs/reports/leather-mold-M1-acceptance-review.md`。视觉复核与实物试压尚未完成，M2/M3 待实现。
...
- **交付范围**：§6 M1 的文件输出、修订链、CLI、GUI 和常规回归均已实现——`domain/leather_molds.py`、`algorithms/leather_mold_pair.py`、
  ...
  `generate-leather-molds`、GUI 模具面板与装配预览。**更正（R1 仅部分修复）**：2026-09-23
  ...
  反例回归（旧实现须错误放行），三份真实候选已重新生成（append-only）。但 R1 代码仍以
  `min_mm` 而非 `conservative_min_mm` 放行，三份下界均小于各自验收门；复验门与修正要求
  见 `docs/reports/leather-mold-M1-acceptance-review.md`。
